  ...
  return (
    <div className="space-y-5 animate-[fadeIn_0.4s_ease-out]">
      <div className="text-center py-6 px-4 rounded-2xl bg-gradient-to-b from-emerald-50 to-stone-50 dark:from-emerald-950/35 dark:to-slate-900 border border-emerald-100 dark:border-emerald-900">
        <p className="text-xs font-semibold text-emerald-700 dark:text-emerald-300 uppercase tracking-widest">Εβδομάδα 3 · Νευροβιολογία</p>
      ...
      <SectionCard title="Διάλεξη (K3 · PPT)">
        ...
        <ul className="space-y-2">
          {topic.progressChecklist.map((line, i) => (
            <li key={i} className="flex gap-3 items-start">
              <input
                id={`w3-${topic.slug}-chk-${i}`}
                type="checkbox"
                checked={!!checklist[i]}
                onChange={() => toggleCheck(i)}
                className="mt-1 size-4 rounded border-slate-300 dark:border-slate-600"
              />
              <label htmlFor={`w3-${topic.slug}-chk-${i}`} className="text-sm text-slate-800 dark:text-slate-100 cursor-pointer">
                {line}
              </label>
            </li>
          ))}
