...
export default function HumanisticPsychologyLesson({ humanisticFlashcards, onMarkFlashSeen }) {
  ...
  const lessonQuizItems = useMemo(() => {
    ...
    return humanisticLessonQuizIds.map((id) => map.get(id)).filter(Boolean)
  }, [])

  const orderedCards = useMemo(() => {
    const order = ['fc-human-1', 'fc-human-2', 'fc-human-3', 'fc-human-4', 'fc-human-5']
    const byId = Object.fromEntries(humanisticFlashcards.map((c) => [c.id, c]))
  ...
  return (
    <div className="space-y-5 animate-[fadeIn_0.4s_ease-out]">
      <div className="text-center py-6 px-4 rounded-2xl bg-gradient-to-b from-emerald-50 to-stone-50 dark:from-emerald-950/35 dark:to-slate-900 border border-emerald-100 dark:border-emerald-900">
        <p className="text-xs font-semibold text-emerald-700 dark:text-emerald-300 uppercase tracking-widest">Ενότητα · SOS</p>
      ...
      <SectionCard title={L.selfActualizationTitle}>
        <p className="text-sm text-slate-700 dark:text-slate-200 whitespace-pre-line leading-relaxed">{L.selfActualization}</p>
      </SectionCard>

      <SectionCard title="Θετική ανθρώπινη φύση">
        <p className="text-sm text-slate-700 dark:text-slate-200 whitespace-pre-line leading-relaxed">{L.positiveNature}</p>
      </SectionCard>

      <SectionCard title="Θεραπεία · clients">
      ...
      <SectionCard title="Κάρτες μνήμης">
        ...
        {currentFc ? (
          <Flashcard
            card={currentFc}
            categoryLabel="Ανθρωπιστική Ψυχολογία"
      ...
      <SectionCard title="Μίνι κουίζ (Σ/Λ & πολλαπλής)">
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
          Οι ίδιες ερωτήσεις μπαίνουν στο κεντρικό κουίζ με φίλτρο «Ανθρωπιστική Ψυχολογία».
        </p>
      ...
      <SectionCard title="Ερωτήσεις ανάπτυξης (εξετάσεις)">
        ...
        <div className="space-y-4">
          {humanisticExamQuestions.map((ex, i) => {
