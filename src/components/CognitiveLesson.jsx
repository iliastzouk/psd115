...
export default function CognitiveLesson({ cognitiveFlashcards, onMarkFlashSeen }) {
  ...
  const lessonQuizItems = useMemo(() => {
    ...
    return cognitiveLessonQuizIds.map((id) => map.get(id)).filter(Boolean)
  }, [])

  const orderedCards = useMemo(() => {
    const byId = Object.fromEntries(cognitiveFlashcards.map((c) => [c.id, c]))
  ...
  return (
    <div className="space-y-3 animate-[fadeIn_0.35s_ease-out]">
      <div className="text-center py-4 px-3 rounded-xl bg-gradient-to-b from-sky-50 to-stone-50 dark:from-sky-950/25 dark:to-slate-900 border border-sky-100 dark:border-sky-900/80">
        <p className="text-[10px] font-semibold text-sky-900 dark:text-sky-300 uppercase tracking-widest">Ενότητα · συμπαγές</p>
      ...
      <SectionCard title={L.cognitiveScienceTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.cognitiveScience}</p>
      </SectionCard>

      <SectionCard title={L.mentalProcessesTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.mentalProcesses}</p>
      </SectionCard>

      <SectionCard title={L.modelsTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.models}</p>
      </SectionCard>
      ...
      <SectionCard title="Ερωτήσεις ανάπτυξης" className={cardCompact}>
        {cognitiveExamQuestions.map((ex, i) => {
