...
export default function Week2TopicLesson({ topic, topicFlashcards, onMarkFlashSeen }) {
  const [checklist, setChecklist] = useState(() => loadWeek2TopicChecklist(topic.slug, topic.progressChecklist.length))
  const [fcIndex, setFcIndex] = useState(0)
  const [openExamIdx, setOpenExamIdx] = useState({})

  useEffect(() => {
    saveWeek2TopicChecklist(topic.slug, checklist)
        ...
        <ul className="space-y-2">
          {topic.progressChecklist.map((line, i) => (
            <li key={i} className="flex gap-3 items-start">
              <input
                id={`w2-${topic.slug}-chk-${i}`}
                type="checkbox"
                checked={!!checklist[i]}
                onChange={() => toggleCheck(i)}
                className="mt-1 size-4 rounded border-slate-300 dark:border-slate-600"
              />
              <label htmlFor={`w2-${topic.slug}-chk-${i}`} className="text-sm text-slate-800 dark:text-slate-100 cursor-pointer">
                {line}
              </label>
            </li>
          ))}
