...
export default function WundtLesson({ wundtFlashcards, onMarkFlashSeen }) {
  ...
  const lessonQuizItems = useMemo(() => {
    ...
    return wundtLessonQuizIds.map((id) => map.get(id)).filter(Boolean)
  }, [])

  const orderedCards = useMemo(() => {
    const order = ['fc-wundt-1', 'fc-wundt-2', 'fc-wundt-3', 'fc-wundt-4', 'fc-wundt-5']
    const byId = Object.fromEntries(wundtFlashcards.map((c) => [c.id, c]))
  ...
  return (
    <div className="space-y-5 animate-[fadeIn_0.4s_ease-out]">
      ...
      <SectionCard title="Πρώτο ψυχολογικό εργαστήριο">
        <p className="text-sm text-slate-700 dark:text-slate-200 whitespace-pre-line leading-relaxed">{L.laboratory}</p>
      </SectionCard>

      <SectionCard title="Συνειδητότητα & υποκειμενική εμπειρία">
        <p className="text-sm text-slate-700 dark:text-slate-200 whitespace-pre-line leading-relaxed">{L.consciousness}</p>
      </SectionCard>

      <SectionCard title={L.introspectionTitle}>
        <p className="text-sm text-slate-700 dark:text-slate-200 whitespace-pre-line leading-relaxed">{L.introspection}</p>
      </SectionCard>
      ...
      <SectionCard title="Μειονέκτημα ενδοσκόπησης">
        <p className="text-sm text-slate-700 dark:text-slate-200 whitespace-pre-line leading-relaxed">{L.weakness}</p>
      </SectionCard>

      <SectionCard title="Πίνακας σύγκρισης">
          ...
          <table className="w-full text-sm border-collapse min-w-[260px]">
            ...
            <tbody>
              {L.comparison.map((row) => (
                <tr key={row.category}>
                  <td className="border border-slate-200 dark:border-slate-600 px-3 py-2 font-medium">{row.category}</td>
      ...
      <SectionCard title="Μίνι κουίζ (Σ/Λ & πολλαπλής)">
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
          Οι ίδιες ερωτήσεις μπαίνουν στο κεντρικό κουίζ με φίλτρο «Δομισμός».
        </p>
