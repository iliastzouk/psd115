...
export default function ThorndikeLesson({ thorndikeFlashcards, onMarkFlashSeen }) {
  ...
  const lessonQuizItems = useMemo(() => {
    ...
    return thorndikeLessonQuizIds.map((id) => map.get(id)).filter(Boolean)
  }, [])

  const orderedCards = useMemo(() => {
    const byId = Object.fromEntries(thorndikeFlashcards.map((c) => [c.id, c]))
  ...
  return (
    <div className="space-y-3 animate-[fadeIn_0.35s_ease-out]">
      ...
      <SectionCard title={L.lawOfEffectTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.lawOfEffect}</p>
      </SectionCard>

      <SectionCard title="Πείραμα" className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.experiment}</p>
      </SectionCard>
      ...
      <SectionCard title="Ερώτηση ανάπτυξης" className={cardCompact}>
        {thorndikeExamQuestions.map((ex, i) => {
