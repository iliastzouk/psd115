...
export default function SocialPsychologyLesson({ socialPsychologyFlashcards, onMarkFlashSeen }) {
  ...
  const lessonQuizItems = useMemo(() => {
    ...
    return socialPsychologyLessonQuizIds.map((id) => map.get(id)).filter(Boolean)
  }, [])

  const orderedCards = useMemo(() => {
    const byId = Object.fromEntries(socialPsychologyFlashcards.map((c) => [c.id, c]))
  ...
  return (
    <div className="space-y-3 animate-[fadeIn_0.35s_ease-out]">
      ...
      <SectionCard title={L.racismTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.racism}</p>
      </SectionCard>

      <SectionCard title={L.aschTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.asch}</p>
      </SectionCard>

      <SectionCard title={L.milgramTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.milgram}</p>
      </SectionCard>
      ...
      <SectionCard title="Ερωτήσεις ανάπτυξης" className={cardCompact}>
        {socialPsychologyExamQuestions.map((ex, i) => {
