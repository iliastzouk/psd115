...
export default function GestaltLesson({ gestaltFlashcards, onMarkFlashSeen }) {
  const L = gestaltLesson
  const catLabel = getCategoryLabel('gestalt')
  const [checklist, setChecklist] = useState(() => loadGestaltChecklist(L.progressChecklist.length))
  const [fcIndex, setFcIndex] = useState(0)
  const [openExamIdx, setOpenExamIdx] = useState({})

  useEffect(() => {
    saveGestaltChecklist(checklist)
  }, [checklist])

  const lessonQuizItems = useMemo(() => {
    const map = new Map(quizQuestions.map((q) => [q.id, q]))
    return gestaltLessonQuizIds.map((id) => map.get(id)).filter(Boolean)
  }, [])

  const orderedCards = useMemo(() => {
    const byId = Object.fromEntries(gestaltFlashcards.map((c) => [c.id, c]))
  ...
  return (
    <div className="space-y-3 animate-[fadeIn_0.35s_ease-out]">
      ...
      <SectionCard title={L.perceptionTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.perception}</p>
      </SectionCard>

      <SectionCard title={L.organizationTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.organization}</p>
      </SectionCard>

      <SectionCard title={L.wholeTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.whole}</p>
      </SectionCard>

      {L.kohlerInsightLecture ? (
        <SectionCard title="Πείραμα Köhler & ενόραση" className={cardCompact}>
      ...
      <SectionCard title="Παγίδα εξέτασης" className={cardCompact}>
        <p className="text-xs font-bold text-teal-900 dark:text-teal-200 mb-1.5">{L.examTrapIntro}</p>
