...
import {
  neuroscienceIntro,
  cognitiveNeuroscienceBlock,
  brocaBlock,
  lashleyBlock,
  neuroscienceExamQuestions,
...
export default function NeuroscienceLesson({ neuroscienceFlashcards, onMarkFlashSeen }) {
  const CN = cognitiveNeuroscienceBlock
  const B = brocaBlock
  const L = lashleyBlock
  const catLabel = getCategoryLabel('neuroscience')
  const [checklist, setChecklist] = useState(() => loadNeuroscienceChecklist(neuroscienceChecklist.length))
  const [fcIndex, setFcIndex] = useState(0)
  const [openExamIdx, setOpenExamIdx] = useState({})

  useEffect(() => {
    saveNeuroscienceChecklist(checklist)
  }, [checklist])

  const lessonQuizItems = useMemo(() => {
    const map = new Map(quizQuestions.map((q) => [q.id, q]))
    return neuroscienceLessonQuizIds.map((id) => map.get(id)).filter(Boolean)
  }, [])

  const orderedCards = useMemo(() => {
    const byId = Object.fromEntries(neuroscienceFlashcards.map((c) => [c.id, c]))
  ...
  return (
    <div className="space-y-3 animate-[fadeIn_0.35s_ease-out]">
      ...
      <SectionCard title="Διάλεξη (PPT)" className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 whitespace-pre-line leading-relaxed font-medium">{neuroscienceIntro.pptSlide}</p>
      </SectionCard>

      <SectionCard title={CN.title} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{CN.theory}</p>
        <p className="text-xs font-bold text-indigo-900 dark:text-indigo-200 mt-3 mb-1">{CN.behavioralNeuroscienceTitle}</p>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{CN.behavioralNeuroscience}</p>
        <p className="text-xs font-bold text-indigo-900 dark:text-indigo-200 mt-3 mb-1">{CN.brainDamageTitle}</p>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{CN.brainDamage}</p>
        <p className="text-xs font-bold text-indigo-900 dark:text-indigo-200 mt-3 mb-1">{CN.brainScanningTitle}</p>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{CN.brainScanning}</p>
        <ul className="list-disc list-inside text-sm text-slate-800 dark:text-slate-100 space-y-1 mt-3">
          {CN.keyPoints.map((k) => (
            <li key={k}>{k}</li>
          ))}
        </ul>
        <p className="text-xs font-bold text-indigo-900 dark:text-indigo-200 mt-3">{CN.examTrapIntro}</p>
        <p className="text-sm text-slate-700 dark:text-slate-200">{CN.examTrapBody}</p>
      </SectionCard>

      <SectionCard title={B.title} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{B.theory}</p>
        <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 mt-3 mb-1">Λειτουργίες περιοχής Broca</p>
        <p className="text-sm text-slate-800 dark:text-slate-100 whitespace-pre-line leading-relaxed">{B.languageFunctions}</p>
        <ul className="list-disc list-inside text-sm text-slate-800 dark:text-slate-100 space-y-1 mt-3">
          {B.keyPoints.map((k) => (
            <li key={k}>{k}</li>
          ))}
        </ul>
        <p className="text-xs font-bold text-indigo-900 dark:text-indigo-200 mt-3">{B.examTrapIntro}</p>
        <p className="text-sm text-slate-700 dark:text-slate-200">{B.examTrapBody}</p>
      </SectionCard>

      <SectionCard title={L.title} className={cardCompact}>
        ...
        <p className="text-xs font-bold text-indigo-900 dark:text-indigo-200 mt-3">{L.examTrapIntro}</p>
        <p className="text-sm text-slate-700 dark:text-slate-200">{L.examTrapBody}</p>
      </SectionCard>

      <SectionCard title="Μνημονικό" className={cardCompact}>
        <MemoryHack lines={neuroscienceMemoryHackLines} title="Γρήγορη σύνδεση" />
      </SectionCard>
      ...
      <SectionCard title="Ερωτήσεις ανάπτυξης" className={cardCompact}>
        {neuroscienceExamQuestions.map((ex, i) => {
      ...
      <SectionCard title="Έλεγχος προόδου" className={cardCompact}>
        <ul className="space-y-2">
          {neuroscienceChecklist.map((label, i) => (
            <li key={label}>
