...
const FC_ORDER = [
  ...
  'fc-educational-psychology-2',
  'fc-educational-psychology-3',
  'fc-educational-psychology-4',
]
...
export default function EducationalPsychologyLesson({ educationalPsychologyFlashcards, onMarkFlashSeen }) {
  ...
  const lessonQuizItems = useMemo(() => {
    ...
    return educationalPsychologyLessonQuizIds.map((id) => map.get(id)).filter(Boolean)
  }, [])

  const orderedCards = useMemo(() => {
    const byId = Object.fromEntries(educationalPsychologyFlashcards.map((c) => [c.id, c]))
  ...
  return (
    <div className="space-y-3 animate-[fadeIn_0.35s_ease-out]">
      <div className="text-center py-4 px-3 rounded-xl bg-gradient-to-b from-amber-50 to-stone-50 dark:from-amber-950/25 dark:to-slate-900 border border-amber-100 dark:border-amber-900/80">
        <p className="text-[10px] font-semibold text-amber-900 dark:text-amber-300 uppercase tracking-widest">Ενότητα · συμπαγές</p>
      ...
      <SectionCard title={L.applicationsTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.applications}</p>
      </SectionCard>

      <SectionCard title={L.rosenthalTitle} className={cardCompact}>
        <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">{L.rosenthal}</p>
      </SectionCard>
      ...
      <SectionCard title="Παγίδα εξέτασης" className={cardCompact}>
        <p className="text-xs font-bold text-amber-900 dark:text-amber-200 mb-1.5">{L.examTrapIntro}</p>
      ...
      <SectionCard title="Ερωτήσεις ανάπτυξης" className={cardCompact}>
        {educationalPsychologyExamQuestions.map((ex, i) => {
